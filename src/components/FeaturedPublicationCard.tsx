...
type FeaturedPublicationCardProps = {
  pub: Publication;
  onContactar: (telefono?: string | number) => void;
  ...
  return <Card className="hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 flex flex-col h-full">
      ...
      <CardContent className="flex flex-col flex-grow p-6 pt-0">
        <div className="flex-grow">
          ...
          <p className="text-gray-600 dark:text-gray-300 mb-4 line-clamp-3 mt-4">
            {pub.descripcion}
          </p>

          <div className="space-y-2 text-sm mb-4">
            <div className="flex items-end gap-2 font-semibold text-2xl text-green-600">
              <span>${pub.precio?.toLocaleString()}</span>
        ...
        <div className="mt-auto">
          <div className="flex gap-2">
            <Button variant="outline" className="flex-1" onClick={() => onViewDetails(pub)}>
              <Eye className="h-4 w-4 mr-2" />
              Ver detalles
            </Button>
            <Button className="flex-1" onClick={() => onContactar(telefonoMostrar)}>
          ...
          }} aria-label="Llamar" className="py-[21px] px-[7px]">
